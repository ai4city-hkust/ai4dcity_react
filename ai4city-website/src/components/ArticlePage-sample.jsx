...
// ==========================================
// 这里通常是 import { ARTICLE_CONTENT } from '../data/articles';
// 为了修复预览错误，我们在此处保留一个结构，
// 你在本地使用时请确保 data/articles.js 存在。
// ==========================================
const MOCK_CONTENT = {
  title: "Markdown Support Preview",
  author: "AI4City Lab",
  date: "2026-03-04",
  img: "/images/news/sample.jpg",
  body: [
    "### 欢迎使用 Markdown 排版\n\n现在你可以使用标准的 Markdown 语法来编写文章正文了：\n\n- **加粗文本** 用于强调重点。\n- *斜体* 用于特殊术语。\n- [超链接](https://ai4city.lab) 用于引用外部资源。\n\n#### 研究背景\n随着城市 3D 建模技术的发展，我们现在可以利用 **Gaussian Splatting** 实现更高效的城市级重建。",
    { type: 'image', src: '/images/research/demo.png', caption: '城市大规模表面重建示例' },
    "以上是一个图文混合的示例，正文部分已经完美支持多级标题、列表和链接。"
  ],
  details: [{ text: "地点：HKUST (GZ)" }, { text: "时间：2026年春季" }]
};

/**
 * 升级版文章详情页 (ArticlePage)
 * 1. 支持 Markdown 渲染 (通过 react-markdown)
 * 2. 增强视觉排版：针对学术文章优化的字体、行间距和标题样式
 * 3. 完美兼容原有 data 结构的 body 数组（字符串部分自动解析为 Markdown）
 */
export const ArticlePage = ({ articleId, setPage, externalData = null }) => {
  // 优先使用传入的数据或从本地库抓取 (本地开发时请取消注释本地库引用)
  // const content = externalData || ARTICLE_CONTENT[articleId] || MOCK_CONTENT;
